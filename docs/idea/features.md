O Elleven-Library é uma ferramenta de gerenciamento e organização de referências visuais (imagens) projetada para artistas e designers, funcionando como uma biblioteca centralizada com recursos avançados de catalogação e busca.
...
* [x] Seleção de diretórios via diálogo do sistema.
* [ ] Suporte a Drag-and-Drop para importação de pastas.


* [x] **Monitoramento em Tempo Real (Folder Watching)**
* [x] Sincronização automática de novos arquivos adicionados às pastas monitoradas.
* [x] Detecção de remoções e renomeações de arquivos no disco.
...
* [x] **Layout Masonry:** Organização otimizada de imagens com diferentes proporções (implementado via motor em Rust/Wasm para alta performance).
...
* [x] Renderização de miniaturas (thumbnails) para navegação rápida.
* [x] Carregamento de imagens em alta definição apenas sob demanda para economia de recursos.
...
* [x] Processamento assíncrono em threads separadas para criar miniaturas sem bloquear a interface.
