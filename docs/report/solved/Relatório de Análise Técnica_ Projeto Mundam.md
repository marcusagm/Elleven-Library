...
### 1.1. Formatos Raster (XCF, MDP, SAI, SAI2) ✅ *Realizado*
...
### 1.2. Formatos Vetoriais (AI, EPS) ✅ *Realizado*
...
*   ✅ *Realizado* - **AI (Illustrator):** O extrator `extract_ai_pdf` é robusto para arquivos modernos, mas falha em arquivos legados.
*   ✅ *Realizado* - **EPS:** A tentativa de usar FFmpeg/Ghostscript é instável em ambientes sem as dependências de sistema corretas.
*   ✅ *Realizado* - **Melhoria:** Integrar uma biblioteca de renderização vetorial leve ou garantir que o fallback para `binary_jpeg` seja a última opção, priorizando a renderização via `pdfium` para o stream PDF interno do AI.
...
1.  ✅ *Realizado* - **Conflito de Foco:** Atalhos globais (como `Cmd+K` para busca) continuam ativos mesmo quando o usuário está digitando em campos de texto, a menos que o componente bloqueie explicitamente.
...
*   ✅ *Realizado* - **Navegação Espacial:** O grid atual usa uma lógica linear. Implementar navegação espacial real (considerando coordenadas X/Y) para lidar com layouts de alvenaria (masonry) de forma mais intuitiva.
...
*   ✅ *Realizado* - **Performance:** A decodificação de imagens XCF/MDP é feita inteiramente na thread principal do Tauri. Mover para `tokio::task::spawn_blocking`.
...
*   ✅ *Realizado* - **Visualização:** O `ImageInspector` carrega a thumbnail mesmo para visualizações grandes. Implementar um sistema de "Dual Loading": carrega a thumbnail instantaneamente e faz o upscale/renderização da imagem completa em background.

---

## 4. Plano de Ação Recomendado

1.  ✅ *Realizado* - **Curto Prazo:** Corrigir os offsets do parser SAI2 e implementar o bloqueio de escopo global no componente `Input`.
2.  ✅ *Realizado* - **Médio Prazo:** Implementar renderização de alta qualidade para MDP (usando as camadas já mapeadas) e XCF.
