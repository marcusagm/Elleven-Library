...
export const Select: Component<SelectProps> = props => {
    ...
    const filteredOptions = createMemo(() => {
        ...
        return local.options.filter(opt => opt.label.toLowerCase().includes(query));
    });
    ...
    // Click outside - bridge both trigger and content to prevent immediate closing on trigger click
    createClickOutside(
    ...
    return (
        <div class={cn('ui-select', local.class)} {...others}>
            ...
            <input type="hidden" name={local.name} value={value()} />
        </div>
    );
};
