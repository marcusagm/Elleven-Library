 ...
 * Fixed to support dynamic positioning on scroll/resize and viewport aware.
 */
export const TagInput: Component<TagInputProps> = props => {
    ...
    const positionContent = () => {
        if (!inputContainerRef || !dropdownRef || !showSuggestions()) return;
    ...
    // Click outside - bridge both container and dropdown
    createClickOutside(
        () => [inputContainerRef, dropdownRef].filter(Boolean) as HTMLElement[],
        () => {
    ...
    createEffect(() => {
        const isVisible = showSuggestions() && filteredSuggestions().length > 0;
        if (isVisible) {
            requestAnimationFrame(positionContent);

            const update = () => requestAnimationFrame(positionContent);
    ...
    return (
        <div class={cn('ui-tag-input-wrapper', local.class)}>
            <div
                ...
                class={cn('ui-tag-input-container', local.disabled && 'ui-tag-input-disabled')}
