...
interface ResizableContextValue {
    ...
    registerPanel: (
        id: string,
        config: { defaultSize: number; minSize: number; maxSize: number }
    ) => void;
...
export const ResizablePanelGroup: Component<ResizablePanelGroupProps> = props => {
    const [local, others] = splitProps(props, ['class', 'direction', 'onLayout', 'children']);
    ...
    return (
        <ResizableContext.Provider value={contextValue}>
            <div
                ref={containerRef}
                class={cn('ui-resizable-group', `ui-resizable-group-${direction()}`, local.class)}
                {...others}
            >
                {local.children}
            </div>
