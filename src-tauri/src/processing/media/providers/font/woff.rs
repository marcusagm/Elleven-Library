...
/// Provider for Web Open Font Format files (.woff).
///
/// This provider handles standard Web Open Font Format (WOFF) version 1 files,
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::font::woff::WoffFontProvider;
///
/// let provider = WoffFontProvider::new();
/// let formats = provider.supported_formats();
///
/// assert_eq!(formats.len(), 1);
/// assert_eq!(formats[0].name, "Web Open Font Format");
/// assert_eq!(formats[0].extensions, vec!["woff"]);
/// ```
#[derive(Default)]
pub struct WoffFontProvider;

impl WoffFontProvider {
    /// Creates a new instance of `WoffFontProvider`.
    ///
    /// # Returns
    ///
    /// `WoffFontProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for WoffFontProvider {
    ...
    /// Validates if the file header matches WOFF magic bytes ("wOFF").
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// `bool` - True if it's a valid WOFF file.
    fn supports_magic_bytes(&self, header_bytes: &[u8]) -> bool {
        header_bytes.starts_with(b"wOFF")
    }
...
#[async_trait]
impl MetadataCapability for WoffFontProvider {
    /// Extracts technical metadata such as family name, weight, and metrics.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the WOFF font file.
    ///
    /// # Errors
    ///
...
#[async_trait]
impl ThumbnailCapability for WoffFontProvider {
