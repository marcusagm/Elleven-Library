...
/// Provider for Mamiya RAW image files (.mef).
///
/// MEF is Mamiya's RAW format used in Mamiya medium-format digital backs.
/// Mamiya (now part of Phase One) produced some of the earliest digital
/// medium-format systems; LibRaw handles MEF decoding.
///
/// # Technical Details
///
/// - **Cameras**: Mamiya Leaf (e.g., Credo, Leaf Digital Backs)
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::image::mamiya::MamiyaRawFormatProvider;
///
/// let provider = MamiyaRawFormatProvider::new();
/// let formats = provider.supported_formats();
///
/// assert_eq!(formats.len(), 1);
/// assert_eq!(formats[0].name, "Mamiya RAW Image");
/// assert_eq!(formats[0].extensions, vec!["mef"]);
/// ```
#[derive(Default)]
pub struct MamiyaRawFormatProvider;

impl MamiyaRawFormatProvider {
    /// Creates a new instance of `MamiyaRawFormatProvider`.
    ///
    /// # Returns
    ///
    /// `MamiyaRawFormatProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for MamiyaRawFormatProvider {
    ...
    /// Validates if the file header matches the Mamiya MEF format.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// `bool` - True if it's a valid Mamiya MEF file.
...
#[async_trait]
impl MetadataCapability for MamiyaRawFormatProvider {
    /// Extracts technical metadata such as dimensions and camera settings.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the Mamiya MEF file.
    ///
    /// # Errors
    ///
    ...
    /// Extracts semantic metadata from the Mamiya MEF file.
    ///
    /// # Arguments
    ///
    /// * `_path` - Path to the Mamiya MEF file (not used in this implementation).
    ///
    /// # Returns
    ///
...
#[async_trait]
impl ThumbnailCapability for MamiyaRawFormatProvider {
    /// Generates a thumbnail from the Mamiya MEF file.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the Mamiya MEF file.
...
#[async_trait]
impl PreviewCapability for MamiyaRawFormatProvider {
