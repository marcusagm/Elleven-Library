...
/// Provider for Panasonic RAW image files (.rw2).
///
/// # Technical Details
/// ## Cameras
/// - Panasonic Lumix DC-S5, S1H, S1, G100, G9, GH5, GH6, G110, G100, G90, G95, G99, G91, G9, GH6, GH5, GH4
/// ## Thumbnail Format
/// - JPEG (from embedded LibRaw preview)
/// ## Metadata
/// - Dimensions (LibRaw) + camera EXIF (rexif)
///
/// # Features
/// - Extracts dimensions and codec information from Panasonic RAW files via LibRaw.
/// - Generates JPEG thumbnails from Panasonic RAW files via LibRaw.
///
/// # Examples
///
/// ```rust
...
///
/// let provider = PanasonicRawFormatProvider::new();
/// let formats = provider.supported_formats();
/// assert!(formats.contains(&MediaType::Image));
/// ```
#[derive(Default)]
pub struct PanasonicRawFormatProvider;

impl PanasonicRawFormatProvider {
    /// Creates a new instance of `PanasonicRawFormatProvider`.
    ///
    /// # Returns
    ///
    /// `PanasonicRawFormatProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for PanasonicRawFormatProvider {
    ...
    /// Checks if the header bytes match Panasonic RAW (RW2) format.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// `bool` - `true` if the header bytes match Panasonic RAW (RW2) format, `false` otherwise.
...
#[async_trait]
impl MetadataCapability for PanasonicRawFormatProvider {
    /// Extracts technical metadata from a Panasonic RAW (RW2) file.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the RW2 file.
    ///
    /// # Returns
    ///
    ...
    /// Extracts semantic metadata from a Panasonic RAW (RW2) file.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the RW2 file.
...
#[async_trait]
impl ThumbnailCapability for PanasonicRawFormatProvider {
    /// Generates a thumbnail for a Panasonic RAW (RW2) file.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the RW2 file.
...
#[async_trait]
impl PreviewCapability for PanasonicRawFormatProvider {
