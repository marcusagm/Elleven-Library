...
/// Provider for Olympus RAW image files (.orf).
///
/// # Technical Details
///
/// - File Format: Olympus RAW (.orf)
/// - Thumbnail Format: WebP image (via FFmpeg)
/// - Metadata: Dimensions via `image-rs`
///
/// # Features
///
/// - Supports `.orf` file extensions.
/// - Extracts technical metadata from RAW files.
/// - Generates thumbnails from RAW files.
///
/// # Examples
///
/// ```rust
...
///
/// let provider = OlympusRawFormatProvider::new();
/// let formats = provider.supported_formats();
/// assert!(formats.contains(&MediaType::Image));
/// ```
#[derive(Default)]
pub struct OlympusRawFormatProvider;

impl OlympusRawFormatProvider {
    /// Creates a new instance of `OlympusRawFormatProvider`.
    ///
    /// # Returns
    ///
...
impl FormatProvider for OlympusRawFormatProvider {
    ...
    /// Checks if the header bytes match the ORF format.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// `bool` - `true` if the header bytes match ORF format, `false` otherwise.
...
#[async_trait]
impl MetadataCapability for OlympusRawFormatProvider {
    /// Extracts technical metadata from a RAW file.
    ///
    /// # Arguments
    ///
    ...
    /// Extracts semantic metadata from a RAW file.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the RAW file.
...
#[async_trait]
impl ThumbnailCapability for OlympusRawFormatProvider {
    /// Generates a thumbnail from a RAW file.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the RAW file.
...
#[async_trait]
impl PreviewCapability for OlympusRawFormatProvider {
