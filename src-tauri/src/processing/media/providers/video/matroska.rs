...
/// Provider for Matroska video files (.mkv).
///
/// Matroska is a flexible, open-standard multimedia container that can hold
/// virtually any combination of video, audio, subtitle and metadata streams.
/// Since browsers do not natively support the MKV container, playback requires
/// HLS transcoding via the streaming server.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::video::matroska::MatroskaVideoProvider;
///
/// let provider = MatroskaVideoProvider::new();
/// let formats = provider.supported_formats();
///
/// assert_eq!(formats[0].name, "Matroska Video");
/// assert_eq!(formats[0].extensions, vec!["mkv"]);
/// ```
#[derive(Default)]
pub struct MatroskaVideoProvider;

impl MatroskaVideoProvider {
    /// Creates a new instance of `MatroskaVideoProvider`.
    ///
    /// # Returns
    ///
    /// `MatroskaVideoProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for MatroskaVideoProvider {
...
#[async_trait]
impl MetadataCapability for MatroskaVideoProvider {
    /// Extracts technical metadata (codecs, dimensions, duration) via FFprobe.
    ///
    /// # Arguments
    ///
    /// * `path` - Path to the Matroska video file.
    ///
    /// # Errors
    ///
...
#[async_trait]
impl ThumbnailCapability for MatroskaVideoProvider {
