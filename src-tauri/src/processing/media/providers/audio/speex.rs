...
/// Provider for Speex Audio files (.spx).
///
/// Speex is an open source, patent-free audio compression format designed for speech.
/// Usually found in Ogg containers. We use Audio HLS for maximum playback compatibility.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::audio::speex::SpeexAudioProvider;
///
/// let provider = SpeexAudioProvider::new();
/// let formats = provider.supported_formats();
/// ```
#[derive(Default)]
pub struct SpeexAudioProvider;

impl SpeexAudioProvider {
    /// Creates a new instance of `SpeexAudioProvider`.
    ///
    /// # Returns
    ///
    /// `SpeexAudioProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for SpeexAudioProvider {
...
#[async_trait]
impl MetadataCapability for SpeexAudioProvider {
