...
/// Provider for OGG Audio files (.ogg, .oga).
///
/// Ogg is a free, open container format from Xiph.Org. Usually contains
/// Vorbis or Opus audio. While some browsers support it, we use Audio HLS
/// to ensure compatibility across all WebViews.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::audio::ogg_audio::OggAudioProvider;
///
/// let provider = OggAudioProvider::new();
/// let formats = provider.supported_formats();
/// ```
#[derive(Default)]
pub struct OggAudioProvider;

impl OggAudioProvider {
    /// Creates a new instance of `OggAudioProvider`.
    ///
    /// # Returns
    ///
    /// `OggAudioProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for OggAudioProvider {
...
#[async_trait]
impl MetadataCapability for OggAudioProvider {
