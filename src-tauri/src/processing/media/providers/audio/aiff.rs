...
/// Provider for AIFF Audio files (.aiff, .aif, .aifc).
///
/// Audio Interchange File Format is an uncompressed audio format developed by Apple.
/// While some browsers support it, we use Audio HLS for maximum compatibility.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::audio::aiff::AiffAudioProvider;
///
/// let provider = AiffAudioProvider::new();
/// let formats = provider.supported_formats();
/// ```
#[derive(Default)]
pub struct AiffAudioProvider;

impl AiffAudioProvider {
    /// Creates a new instance of `AiffAudioProvider`.
    ///
    /// # Returns
    ///
    /// `AiffAudioProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for AiffAudioProvider {
...
#[async_trait]
impl MetadataCapability for AiffAudioProvider {
