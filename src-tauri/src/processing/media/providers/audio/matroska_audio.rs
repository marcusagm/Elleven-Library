...
/// Provider for Matroska Audio files (.mka).
///
/// Audio-only Matroska container format. Requires transcoding via Audio HLS
/// to play in the browser since the container is unsupported.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::audio::matroska_audio::MatroskaAudioProvider;
///
/// let provider = MatroskaAudioProvider::new();
/// let formats = provider.supported_formats();
/// ```
#[derive(Default)]
pub struct MatroskaAudioProvider;

impl MatroskaAudioProvider {
    /// Creates a new instance of `MatroskaAudioProvider`.
    ///
    /// # Returns
    ///
    /// `MatroskaAudioProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for MatroskaAudioProvider {
...
#[async_trait]
impl MetadataCapability for MatroskaAudioProvider {
