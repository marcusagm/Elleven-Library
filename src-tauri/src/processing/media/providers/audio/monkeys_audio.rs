...
/// Provider for Monkey's Audio files (.ape).
///
/// Monkey's Audio is a lossless audio compression format. Not supported natively
/// by WebViews, so we transcode via Audio HLS.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::audio::monkeys_audio::MonkeysAudioProvider;
///
/// let provider = MonkeysAudioProvider::new();
/// let formats = provider.supported_formats();
/// ```
#[derive(Default)]
pub struct MonkeysAudioProvider;

impl MonkeysAudioProvider {
    /// Creates a new instance of `MonkeysAudioProvider`.
    ///
    /// # Returns
    ///
    /// `MonkeysAudioProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for MonkeysAudioProvider {
...
#[async_trait]
impl MetadataCapability for MonkeysAudioProvider {
