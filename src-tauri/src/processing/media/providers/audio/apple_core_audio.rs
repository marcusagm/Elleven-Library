...
/// Provider for Apple Core Audio files (.caf).
///
/// Core Audio Format developed by Apple to overcome limitations of older formats like AIFF/WAV.
/// Usually requires Audio HLS for playback in browsers depending on the encapsulated codec.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::audio::apple_core_audio::AppleCoreAudioProvider;
///
/// let provider = AppleCoreAudioProvider::new();
/// let formats = provider.supported_formats();
/// ```
#[derive(Default)]
pub struct AppleCoreAudioProvider;

impl AppleCoreAudioProvider {
    /// Creates a new instance of `AppleCoreAudioProvider`.
    ///
    /// # Returns
    ///
    /// `AppleCoreAudioProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for AppleCoreAudioProvider {
...
#[async_trait]
impl MetadataCapability for AppleCoreAudioProvider {
