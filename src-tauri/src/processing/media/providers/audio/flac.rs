...
/// Provider for FLAC Audio files (.flac).
///
/// Handles Free Lossless Audio Codec files. Browsers generally support
/// FLAC playback natively now, so we use Native strategy for parity with V1.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::audio::flac::FlacAudioProvider;
///
/// let provider = FlacAudioProvider::new();
/// let formats = provider.supported_formats();
/// ```
#[derive(Default)]
pub struct FlacAudioProvider;

impl FlacAudioProvider {
    /// Creates a new instance of `FlacAudioProvider`.
    ///
    /// # Returns
    ///
    /// `FlacAudioProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for FlacAudioProvider {
...
#[async_trait]
impl MetadataCapability for FlacAudioProvider {
