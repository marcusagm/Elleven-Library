...
/// Provider for Audible Audio files (.aax).
///
/// Audible's enhanced audio format. Often contains DRM-protected AAC or similar.
/// FFmpeg can sometimes transcode them if they aren't fully encrypted, handled via Audio HLS.
///
/// # Technical Details
///
...
///
/// ```no_run
/// use mundam_lib::processing::media::providers::audio::audible::AudibleAudioProvider;
///
/// let provider = AudibleAudioProvider::new();
/// let formats = provider.supported_formats();
/// ```
#[derive(Default)]
pub struct AudibleAudioProvider;

impl AudibleAudioProvider {
    /// Creates a new instance of `AudibleAudioProvider`.
    ///
    /// # Returns
    ///
    /// `AudibleAudioProvider` - A new instance of the provider.
    pub fn new() -> Self {
        Self
    }
}

impl FormatProvider for AudibleAudioProvider {
...
#[async_trait]
impl MetadataCapability for AudibleAudioProvider {
