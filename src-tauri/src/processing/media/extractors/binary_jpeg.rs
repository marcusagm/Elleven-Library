...
pub fn scan_mmap_for_jpeg(mmap: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    best.map(|(s, l)| mmap[s..s+l].to_vec()).ok_or_else(|| "No JPEG found".into())
}
...
pub fn scan_mmap_for_png(mmap: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    best.map(|(s, l)| mmap[s..s+l.min(mmap.len()-s)].to_vec()).ok_or_else(|| "No PNG found".into())
}
...
pub fn scan_mmap_for_tiff(mmap: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    for i in 0..limit.saturating_sub(4) {
        if mmap[i..].starts_with(TIFF_LE) || mmap[i..].starts_with(TIFF_BE) {
            let end = if i == 0 { mmap.len() } else { (i + 50 * 1024 * 1024).min(mmap.len()) };
...
pub fn extract_xmp_thumbnail(path: &Path) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    if let Some(s_pos) = buf.windows(start_m.len()).position(|w| w == start_m) {
        ...
        if let Some(e_rel) = buf[content_s..].windows(end_m.len()).position(|w| w == end_m) {
            let content_e = content_s + e_rel;
            let raw_str = String::from_utf8_lossy(&buf[content_s..content_e]);
            let clean: String = raw_str.replace("&#xA;", "").replace("&#xD;", "")
                .chars().filter(|c| c.is_alphanumeric() || *c == '+' || *c == '/' || *c == '=').collect();
            ...
            if decoder.decode_headers().is_ok() {
                if let Some(info) = decoder.info() {
                    ...
                    if let Ok(pix) = decoder.decode() {
                        let mut png = Vec::new();
                        let et = if pix.len() == (w * h * 4) as usize { image::ExtendedColorType::Rgba8 }
                                 else if pix.len() == (w * h) as usize { image::ExtendedColorType::L8 }
                                 else { image::ExtendedColorType::Rgb8 };
                        if image::codecs::png::PngEncoder::new(std::io::Cursor::new(&mut png)).write_image(&pix, w, h, et).is_ok() {
                            return Ok(png);
                        }
                    }
                }
