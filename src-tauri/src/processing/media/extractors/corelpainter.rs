...
pub fn extract_corel_painter_preview(path: &Path) -> Result<(Vec<u8>, String), Box<dyn std::error::Error>> {
    ...
    if let Some(start_index) = buffer.windows(4).position(|window| window == start_signature) {
        let end_signature = [0xFF, 0xD9];
        if let Some(end_relative_index) = buffer[start_index..].windows(2).position(|window| window == end_signature) {
            let end_index = start_index + end_relative_index + 2;
            return Ok((buffer[start_index..end_index].to_vec(), "image/jpeg".to_string()));
        }
    }
    Err("No embedded JPEG found".into())
}

pub fn extract_corelpainter_metadata(path: &Path) -> Result<serde_json::Value, Box<dyn std::error::Error>> {
