...
pub fn generate_raw_thumbnail(
    ...
    // This is MUCH faster for large RAW files (20MB-100MB+) as it avoids allocation and copy
    let mmap = unsafe { memmap2::MmapOptions::new().map(&file)? };

    let mut raw = rsraw::RawImage::open(&mmap)
        .map_err(|e| format!("LibRaw open error: {:?}", e))?;

    // Try to extract the embedded thumbnail first (fastest)
    // unpack() extracts the preview image from the RAW container
    // Try to extract the embedded thumbnail first (fastest)
    // We do NOT need to call unpack() (which decodes RAW data) to get thumbnails.
    // extract_thumbs() works on the opened file structure.
    ...
    if let Some(thumb) = thumbs.iter().max_by_key(|t| t.width * t.height) {
        ...
        let img = load_from_memory(&thumb.data)
        ...
        Ok(())
            ...
            // If no thumbnail found, we MIGHT try unpack() and full process, but that's very slow.
            // For now, failure is better than hanging the UI for seconds.
