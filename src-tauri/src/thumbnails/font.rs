...
pub fn generate_font_thumbnail(
    ...
    if ext == "woff" {
        ...
        let decoded =
            wuff::decompress_woff1(&data).map_err(|e| format!("WOFF1 decode failed: {:?}", e))?;
        fontdb.load_font_source(usvg::fontdb::Source::Binary(Arc::new(decoded)));
    } else if ext == "woff2" {
        let data = std::fs::read(input_path)?;
        let decoded =
            wuff::decompress_woff2(&data).map_err(|e| format!("WOFF2 decode failed: {:?}", e))?;
        fontdb.load_font_source(usvg::fontdb::Source::Binary(Arc::new(decoded)));
    } else {
        fontdb
            .load_font_file(input_path)
    ...
    // 5. Parse SVG
    let tree =
        usvg::Tree::from_str(&svg_content, &opt).map_err(|e| format!("SVG parse error: {}", e))?;
