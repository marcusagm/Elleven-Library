        ...
        for ty in 0..tiles_y {
            for tx in 0..tiles_x {
                ...
                reader.seek(SeekFrom::Start(
                    lptr + 8 + ((ty * tiles_x + tx) * bytes_per_offset) as u64,
                ))?;
    ...
    for ly in 0..tile_h {
        for lx in 0..tile_w {
            ...
            if out_a > 0 {
                ...
                canvas_data[canvas_idx + 1] =
                    ((sg * sa + dg * da * (255 - sa) / 255) / out_a) as u8;
                canvas_data[canvas_idx + 2] =
                    ((sb * sa + db * da * (255 - sa) / 255) / out_a) as u8;
