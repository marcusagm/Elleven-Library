    ...
    for i in 0..archive.len() {
        if let Ok(entry) = archive.by_index(i) {
            ...
            if name.ends_with(".png")
                && (name.contains("preview") || name.contains("page") || name.contains("thumb"))
                && entry.size() > max_size
            {
                max_size = entry.size();
...
fn construct_bmp_from_dib(dib_data: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    if dib_data.len() < header_size_usize {
    ...
    while i < len.saturating_sub(16) {
        ...
        if buffer[i] == 0x42 && buffer[i + 1] == 0x4D {
            ...
            if declared_size > 50 && (i + declared_size) <= len {
                ...
                if (14..10000).contains(&offset_val) {
...
fn wrap_raw_1bit_bmp(
    ...
    bmp.write_all(&info_header_size.to_le_bytes())?;
